...
Shader "Reflex Shader 2/Reflex Shader 2 Cutout"
{
	...
	SubShader
	{
		...
		void outlineVertexDataFunc( inout appdata_full v, out Input o )
		{
			...
			float2 uv_OutlineMask389_g100 = v.texcoord;
			float outlineVar = ( (0.0 + (_OutlineWidth - 0.0) * (0.002 - 0.0) / (1.0 - 0.0)) * tex2Dlod( _OutlineMask, float4( uv_OutlineMask389_g100, 0, 0.0) ) ).r;
		...
		void outlineSurf( Input i, inout SurfaceOutput o )
		{
			...
			float4 temp_cast_1 = (grayscale611_g100).xxxx;
			float4 lerpResult745_g100 = lerp( DiffuseBase606_g100 , temp_cast_1 , _GrayscaleColor);
			...
			float3 appendResult387_g100 = (float3(_OutlineColor.r , _OutlineColor.g , _OutlineColor.b));
			...
			if( _WorldSpaceLightPos0.w <= 0.0 )
				...
				ifLocalVar196_g100 = ( 1 * ase_lightColor );
			float4 temp_cast_6 = (_AmbientMinimum).xxxx;
			float4 clampResult650_g100 = clamp( ( float4( localFunction_ShadeSH9411_g100 , 0.0 ) + ifLocalVar196_g100 ) , temp_cast_6 , float4( 1,1,1,1 ) );
			float4 temp_cast_7 = (1.0).xxxx;
			float4 lerpResult723_g100 = lerp( saturate( clampResult650_g100 ) , temp_cast_7 , _Unlit);
			...
			float lerpResult726_g100 = lerp( temp_output_335_0_g100 , saturate( ( temp_output_335_0_g100 * 1 ) ) , _ObjectShadow);
			...
			float4 temp_cast_12 = (temp_output_351_0_g100).xxxx;
			float4 temp_cast_14 = (temp_output_351_0_g100).xxxx;
			float div354_g100=256.0/float((int)255.0);
			float4 posterize354_g100 = ( floor( temp_cast_14 * div354_g100 ) / div354_g100 );
			float4 lerpResult720_g100 = lerp( temp_cast_12 , posterize354_g100 , _PosterizeToggle);
			float4 temp_cast_15 = (0.0).xxxx;
			float4 temp_cast_16 = (1.0).xxxx;
			float4 clampResult364_g100 = clamp( lerpResult720_g100 , temp_cast_15 , temp_cast_16 );
			...
			float4 temp_cast_17 = (temp_output_353_0_g100).xxxx;
			float4 temp_cast_19 = (temp_output_353_0_g100).xxxx;
			float div356_g100=256.0/float((int)255.0);
			float4 posterize356_g100 = ( floor( temp_cast_19 * div356_g100 ) / div356_g100 );
			float4 lerpResult721_g100 = lerp( temp_cast_17 , posterize356_g100 , _PosterizeToggle);
			float4 temp_cast_20 = (0.0).xxxx;
			float4 temp_cast_21 = (1.0).xxxx;
			float4 clampResult363_g100 = clamp( lerpResult721_g100 , temp_cast_20 , temp_cast_21 );
			...
			float4 lerpResult735_g100 = lerp( temp_cast_8 , lerpResult372_g100 , _HalfLambertToggle);
			...
			float lerpResult718_g100 = lerp( 0.0 , ( _OutlineColor.a * Opacity674_g100 ) , _OutlineToggle);
			o.Emission = ( ( float4( ( (Diffuse105_g100).rgb * appendResult387_g100 ) , 0.0 ) * Lighting201_g100 * Shadow375_g100 ) + Emission679_g100 ).rgb;
			clip( lerpResult718_g100 - _CutoutThreshold );
...
/*ASEBEGIN
Version=17000
1927;12;1906;1020;964.7664;658.65;1.164724;True;False
